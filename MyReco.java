...
class MyReco {
	...
	public static void MeshFeature(PgmImage pgm, double w[]) {
		...
		//メッシュ盗聴料の計測(始)
	        int[] b = new int[K];//メッシュ特徴量計測用: 面積s[K], 黒画素数b[K]
	        int[] s = new int[K];
	        ...
	        for(k=0; k<K; k++){
			if(s[k]>0) w[k] = (double)b[k]/s[k];//各メッシュの黒画素の割合を求めてw[k]に代入
			else w[k] = 0.0;
		}
		//メッシュ特徴量の計測(終)
	}
	...
	public static int Reco(PgmImage pgm) {  
		...
		double pv1[][] = {//メッシュ数7*7の標準パターン特徴ベクトル
		...
		MeshFeature(pgm, w1);//文字画像のメッシュ特徴量を求める
		
		
		double dmin = (double)LARGE_VALUE; //最小距離の初期化
		double dx; //入力パターンとの距離
		double sum = 0;

		/***担当箇所(始)***/
		double[] meshArray = new double[C];//pv1[][]とw1[]の距離の配列
		...
		double weight = 50;//重み meshArray[]の値はcrossArray[]に比べて非常に小さいため
		for(int k=0; k<C; k++){     
